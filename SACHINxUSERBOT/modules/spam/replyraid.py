...
from SACHINxUSERBOT.helper.utility import get_arg
...
from SACHINxUSERBOT.database.rraid import *
from SACHINxUSERBOT import SUDO_USER
...
async def pornspam(xspam: Client, e: Message): 
    ...
    count = int(counts)
    for _ in range(count):
         await xspam.send_message(e.chat.id, SACHINxUSERBOT)
...
async def raid(xspam: Client, e: Message):  
      SACHINxUSERBOT = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(SACHINxUSERBOT) == 2:
          counts = int(SACHINxUSERBOT[0])
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**Sorry !! i Can't Spam Here.**")
          ok = await xspam.get_users(SACHINxUSERBOT[1])
          id = ok.id
#          try:
